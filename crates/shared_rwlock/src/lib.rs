...
pub use self::errors::LockError;
pub use self::poison::{LockResult, PoisonError, TryLockError, TryLockResult};
use self::sys::shared_memory;
...
impl<T: ?Sized> Drop for SharedRwLock<T> {
    fn drop(&mut self) {
        ...
        let _ = shared_memory::close(unsafe { self.handle.as_mut() }, ptr);
    }
}
...
impl<T> SharedRwLock<T> {
    ...
    pub fn new(shared_id: &windows::core::HSTRING, len: usize) -> Result<(Self, bool), LockError> {
        let size = RWLOCK_LOCK_STATE_SIZE + size_of::<T>() * len;
        let ((handle, view), is_created) = shared_memory::open(shared_id, size)
            .map(|pair| (pair, false))
            .or_else(|_| shared_memory::create(shared_id, size).map(|pair| (pair, true)))?;

        Ok((
            Self {
                handle: NonNull::new(handle.0).unwrap(),
                len,
                shared: NonNull::new(view.Value.cast::<SharedCell<T>>()).unwrap(),
            },
            is_created,
        ))
    }
...
impl<'a, T: ?Sized> RwLockReadGuard<'a, T> {
        ...
        U: ?Sized,
    {
        ...
        MappedRwLockReadGuard { data, inner_lock: orig.inner_lock, len: orig.len }
    }
        ...
        U: ?Sized,
    {
        ...
        match f(unsafe { orig.data.as_ref() }) {
            Some(data) => {
                ...
                let orig = ManuallyDrop::new(orig);
                Ok(MappedRwLockReadGuard { data, inner_lock: orig.inner_lock, len: orig.len })
            }
            None => Err(orig),
        }
    }
